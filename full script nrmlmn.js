...
await ACtl.pubVar('track', t) ;
let track = await ACtl.pubVar('track') ;

await ACtl.on('tabLoadEnd', '#currentTab') ;
//Now we can do our thing
ACtl.runInTab('#currentTab', ()=>{
	//alert("tab is loaded, We are in. Let's do some sneaky stuff.") ;
	//Functions
...
ACtl.runInTab('#currentTab', ()=>{
	...
	alert("jumping between tabs work now we need public var"+track); //track not working public var :(
	document.getElementById("tracking").value = t;
	
//alert("niggers man");
	document.getElementsByClassName("btn btn-success")[0].click(); //Ajouter a la liste
	document.getElementById("sendData").click(); // Creer le sac
});
  // your code here
